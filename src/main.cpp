...
#if defined(NDEBUG)
# error "Kkachicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3e177ef9a1a877c24f7a3fb6b7ea12e00aace8a73bd18a2d7920e36af80e890e");
uint256 hashGenesisBlockMerkleRoot("0x94ee8655905364f41cf9678fb0e97bff41beaf466121692f1a8900ce7f7b92ab");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kkachicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kkachicoin Signed Message:\n";
...
{
    // Kkachicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 2592000); // Kkachicoin: 2,592k blocks in ~6 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60;	// Kkachicoin: retarget every 1 hour
static const int64 nTargetSpacing = 12;			// Kkachicoin: 12 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kkachicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    /*
    // Kkachicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kkachicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x075c618539f14e8cd6b87ee5931fcfcd6811907b0297fe5e45c7b7932ee70325");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "My first kkachi 14:31 Wed, 11 Jul 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 32 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a24d7fe5b61bfdf1431d91f1e0064b88ced293275ded89fa91907b0775c20dc82bb5a8831973aab034934333ee9b4d9eff421a17d5ad560caf42488dcaf73029") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1531287238;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 129345669;

        if (fTestNet)
        {
            block.nTime    = 1531287174;
            block.nNonce   = 114147763;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x94ee8655905364f41cf9678fb0e97bff41beaf466121692f1a8900ce7f7b92ab"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Kkachicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KkachicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KkachicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KkachicoinMiner(CWallet *pwallet)
{
    printf("KkachicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kkachicoin-miner");
    ...
    try { loop {
        ...
        printf("Running KkachicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KkachicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KkachicoinMiner, pwallet));
}
